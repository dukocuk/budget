...
Rent,5000,monthly,Jan,Dec,60000`;
    ...
    it('should handle UTF-8 BOM', () => {
      const csvContent =
        '\ufeffUdgift,Beløb,Frekvens,Start Måned,Slut Måned\nTest,100,monthly,1,12';
    ...
    it('should return error for CSV without expense header', () => {
      ...
      expect(result.errors).toContain(
        'Kunne ikke finde udgiftsdata i CSV filen'
      );
    });
  });
...
Test,100`;
      ...
      expect(result.success).toBe(false);
      expect(
        result.errors.some(err => err.includes('Mangler påkrævede felter'))
      ).toBe(true);
    });
...
,100,monthly,1,12`;
      ...
      expect(result.success).toBe(false);
      expect(
        result.errors.some(err => err.includes('Udgiftsnavn er påkrævet'))
      ).toBe(true);
    });
  });
...
describe('validateImport', () => {
  ...
  it('should handle mixed valid and duplicate expenses', () => {
    ...
    expect(result.valid.map(e => e.name)).toEqual([
      'New Expense',
      'Another New',
    ]);
