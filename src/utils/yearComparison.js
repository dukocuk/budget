...
export function comparePeriods(period1, period2) {
  ...
  const totalAnnualChange = summary1.totalAnnual !== 0
    ? ((totalAnnualDiff / summary1.totalAnnual) * 100)
    : 0
  const avgMonthlyChange = summary1.avgMonthly !== 0
    ? ((avgMonthlyDiff / summary1.avgMonthly) * 100)
    : 0
  const monthlyBalanceChange = summary1.monthlyBalance !== 0
    ? ((monthlyBalanceDiff / Math.abs(summary1.monthlyBalance)) * 100)
    : 0
  const annualReserveChange = summary1.annualReserve !== 0
...
export function compareMonthlyTotals(expenses1, expenses2) {
  ...
  return monthly1.map((amount1, index) => ({
    ...
    percentageChange: amount1 !== 0 ? ((monthly2[index] - amount1) / amount1 * 100) : 0
  }))
}
...
export function compareExpenses(expenses1, expenses2) {
  ...
  const modified = expenses2.filter(e2 => {
    const e1 = expenses1.find(e => e.name.toLowerCase() === e2.name.toLowerCase())
    ...
    return {
      ...
      percentageChange: e1.amount !== 0 ? ((e2.amount - e1.amount) / e1.amount * 100) : 0
    }
  })
