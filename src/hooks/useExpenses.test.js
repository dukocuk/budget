...
describe('useExpenses', () => {
  ...
  describe('Undo/Redo Workflow Integration', () => {
    ...
    it('should support update → undo → redo workflow', async () => {
      ...
      // Update the expense
      await act(async () => {
        await result.current.updateExpense('test-expense-1', {
          name: 'Updated Name',
        });
      });

      await waitFor(() => {
        const expense = result.current.expenses.find(
          e => e.id === 'test-expense-1'
        );
    ...
    it('should support delete → undo → redo workflow', async () => {
      ...
      await waitFor(() => {
        expect(result.current.expenses).toHaveLength(initialCount);
        const restored = result.current.expenses.find(
          e => e.id === 'test-expense-1'
        );
