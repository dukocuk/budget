...
export const useBudgetPeriods = (userId) => {
  ...
  const loadPeriods = useCallback(async () => {
    ...
    try {
      ...
      const active = loadedPeriods.find(p => p.status === 'active') || loadedPeriods[0] || null
  ...
  const updatePeriod = useCallback(async (id, updates) => {
    ...
    try {
      ...
      if (updates.monthlyPayments !== undefined) {
        ...
        values.push(updates.monthlyPayments ? JSON.stringify(updates.monthlyPayments) : null)
      }
   ...
   */
  const archivePeriod = useCallback(async (id) => {
   ...
   */
  const calculateEndingBalance = useCallback(async (periodId) => {
    if (!userId) return 0
    ...
    try {
      ...
      const endingBalance = period.previous_balance + totalIncome - totalExpenses
   ...
   */
  const saveAsTemplate = useCallback(async (periodId, templateName, templateDescription = '') => {
    if (!userId) return
  ...
  const createFromTemplate = useCallback(async (templateData) => {
    ...
    try {
      ...
      if (selectedExpenseIds && selectedExpenseIds.length > 0) {
        ...
        await copySelectedExpenses(userId, templateId, newId, selectedExpenseIds)
      } else {
        // Copy all expenses
   ...
   */
  const getExpensesForPeriod = useCallback(async (periodId) => {
    ...
    try {
      ...
      return {
        ...
        monthlyPayments: period.monthly_payments ? JSON.parse(period.monthly_payments) : null,
 ...
 */
async function copySelectedExpenses(userId, fromPeriodId, toPeriodId, expenseIds) {
  try {
    if (!expenseIds || expenseIds.length === 0) return

    // Get selected expenses from source period
    const placeholders = expenseIds.map((_, index) => `$${index + 3}`).join(', ')
    const result = await localDB.query(
    ...
    logger.info(`✅ Copied ${result.rows.length} selected expenses to new period`)
  } catch (err) {
