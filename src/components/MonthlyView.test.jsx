...
describe('MonthlyView', () => {
  ...
  describe('Loading State', () => {
    it('should show loading message when expenses are loading', () => {
      ...
      expect(screen.getByText('Indlæser månedsoversigt...')).toBeInTheDocument()
    })
  })
  ...
  describe('Monthly Amounts', () => {
    ...
    it('should display quarterly amounts only in quarter months', () => {
      ...
      const amountCells = container.querySelectorAll('tbody tr:not(.total-row) .amount-cell')
      const cellsWith500 = Array.from(amountCells).filter(cell => cell.textContent === '500')
    ...
    it('should display yearly amount only in specified month', () => {
      ...
      const amountCells = container.querySelectorAll('tbody tr:not(.total-row) .amount-cell')
      const cellsWith3000 = Array.from(amountCells).filter(cell => cell.textContent === '3.000')
  ...
  describe('Row Totals', () => {
    it('should display annual total for each expense', () => {
      ...
      const totalCells = container.querySelectorAll('tbody tr:not(.total-row) .total-cell')
      const hasCorrectTotal = Array.from(totalCells).some(cell => cell.textContent === '1.200')
  ...
  describe('Formatting', () => {
    it('should format amounts with Danish locale', () => {
      ...
      const hasFormattedNumber = Array.from(allCells).some(cell => cell.textContent === '10.000')
  ...
  describe('Complex Scenarios', () => {
    it('should handle partial year expenses correctly', () => {
      ...
      const has600Total = Array.from(totalCells).some(cell => cell.textContent === '600')
