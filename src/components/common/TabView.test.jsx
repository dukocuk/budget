...
describe('TabView', () => {
  ...
  describe('Active Tab State', () => {
    ...
    it('should display correct content for active tab', () => {
      const { rerender } = render(
        <TabView tabs={mockTabs} activeTab={0} onTabChange={vi.fn()} />
      );
      expect(screen.getByText('Overview Content')).toBeInTheDocument();
  ...
  describe('Tab Interaction', () => {
    it('should call onTabChange when tab is clicked', () => {
      ...
      render(
        <TabView tabs={mockTabs} activeTab={0} onTabChange={handleTabChange} />
      );
  ...
  describe('Dropdown Tabs', () => {
    ...
    it('should show dropdown arrow for tabs with dropdown items', () => {
      render(
        <TabView tabs={dropdownTabs} activeTab={0} onTabChange={vi.fn()} />
      );
    ...
    it('should not show dropdown arrow for regular tabs', () => {
      render(
        <TabView tabs={dropdownTabs} activeTab={1} onTabChange={vi.fn()} />
      );
    ...
    it('should show dropdown on mouse enter', () => {
      ...
      expect(
        screen.getByText('Line Chart', { selector: '.dropdown-label' })
      ).toBeInTheDocument();
      expect(
        screen.getByText('Pie Chart', { selector: '.dropdown-label' })
      ).toBeInTheDocument();
    });
    ...
    it('should hide dropdown on mouse leave', () => {
      ...
      expect(
        screen.queryByText('Line Chart', { selector: '.dropdown-label' })
    ...
    it('should show first dropdown item content by default', () => {
      ...
      expect(
        screen.getByText('Line Chart', { selector: 'div' })
      ).toBeInTheDocument();
    });
    ...
    it('should change dropdown item when clicked', () => {
      ...
      const pieChartItem = screen.getByText('Pie Chart', {
        selector: '.dropdown-label',
      });
      fireEvent.click(pieChartItem.closest('button'));

      expect(
        screen.getByText('Pie Chart', { selector: 'div' })
      ).toBeInTheDocument();
      expect(
        screen.queryByText('Line Chart', { selector: 'div' })
      ).not.toBeInTheDocument();
    });
    ...
    it('should mark selected dropdown item', () => {
      ...
      const lineChartItem = screen
        .getByText('Line Chart', { selector: '.dropdown-label' })
        .closest('button');
      ...
      const lineChartItemAfter = screen
        .getByText('Line Chart', { selector: '.dropdown-label' })
        .closest('button');
      ...
      fireEvent.click(
        screen
          .getByText('Pie Chart', { selector: '.dropdown-label' })
          .closest('button')
      );
      fireEvent.mouseEnter(wrapper); // Re-open dropdown

      const pieChartItem = screen
        .getByText('Pie Chart', { selector: '.dropdown-label' })
        .closest('button');
    ...
    it('should close dropdown when item is clicked', () => {
      ...
      expect(
        screen.queryByText('Pie Chart', { selector: '.dropdown-label' })
      ).not.toBeInTheDocument();
    });
  });
  ...
  describe('Accessibility', () => {
    ...
    it('should have aria-selected attribute', () => {
      ...
      expect(screen.getByLabelText('Expenses')).toHaveAttribute(
        'aria-selected',
        'false'
      );
